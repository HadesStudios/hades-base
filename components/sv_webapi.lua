...
COMPONENTS.WebAPI = {
	...
	Validate = function(self)
		COMPONENTS.Logger:Trace("Core", "Validating API Key With HadesStudios Services", {
			console = true,
		})
		...
		if res.code ~= 200 then
			...
			if COMPONENTS.Config.Server.Access then
				COMPONENTS.Logger:Trace(
					"Core",
					string.format(
						"Server ^2#%s^7 - ^2%s^7 HadesStudios, Running With Access Restrictions",
				...
				COMPONENTS.Logger:Info(
					"Core",
					string.format(
						"Server ^2#%s^7 - ^2%s^7 HadesStudios, Running With No Access Restriction",
						tostring(COMPONENTS.Config.Server.ID),
						COMPONENTS.Config.Server.Name
					),
					{ console = true }
